...
class Post(authorListing: ActorRef) extends PersistentActor with ActorLogging {
  ...
  override def preStart(): Unit = {
    super.preStart()
    log.info(s"   ***   preStart : Post from $from")
  }

  override def postStop(): Unit = {
    super.postStop()
    log.info(s"   ***   postSop : Post from $from")
  }

  override def receiveRecover: Receive = {
    case evt: PostAdded =>
      log.info("   ***   recovery : PostAdded")
      context.become(created)
      state = state.updated(evt)
    case evt @ PostPublished =>
      log.info("   ***   recovery : PostPublished")
      context.become(published)
      state = state.updated(evt)
    case evt: Event =>
      log.info("   ***   recovery : Event")
      state = state.updated(evt)
  }
  ...
  def created: Receive = {
    ...
    case Publish(postId) =>
      persist(PostPublished) { evt =>
        ...
        if (fsmTestMode)
          authorListing ! AuthorListingFSM.PostSummary(c.author, postId, c.title)
        else
